...
st.set_page_config(page_title="ALPHA TERMINAL v4.2", layout="wide")
...
<style>
    ...
    .ledger-box { background-color: #161b22; border: 1px solid #30363d; padding: 20px; margin-top: 20px; }
</style>
...
def get_yc_analysis():
    try:
        ...
        # 100% = Healthy, 50% = Inverted, 0% = Recession Trap (Danger)
        score = 0 if (curr > 0 and was_inv) else (50 if curr < 0 else 100)
...
def run_model():
    ...
    # [1] Trend: SPY vs 200MA (100% if above)
    spy_ma = float(spy.rolling(200).mean().iloc[-1])
    tr_p = 100 if spy_px > spy_ma else 0
    
    # [2] Credit: HYG/IEF vs 50MA (100% if Risk-On)
    ratio = prices['HYG'].dropna() / prices['IEF'].dropna()
    cr_p = 100 if float(ratio.iloc[-1]) > float(ratio.rolling(50).mean().iloc[-1]) else 0
    
    # [3] Breadth: 11 Sectors vs 200MA (100% if all sectors participating)
    secs = ['XLY','XLP','XLE','XLF','XLV','XLI','XLB','XLK','XLU','XLC','XLRE']
    above = 0
    for s in secs:
        p = prices[s].dropna()
        if not p.empty and p.iloc[-1] > p.rolling(200).mean().iloc[-1]: above += 1
    br_p = (above / 11) * 100 # Direct strength mapping
    
    # [4] VIX: 1.5 SD (100% if Panic/Oversold, 0% if Complacent/Overbought)
    vix = prices['^VIX'].dropna()
    vx_p, v_px, v_u = 50, 0.0, 0.0
    if len(vix) > 21:
        v_px = float(vix.iloc[-1])
        v_ma = vix.rolling(20).mean().iloc[-1]
        v_std = vix.rolling(20).std().iloc[-1]
        v_u = v_ma + (1.5 * v_std); v_l = v_ma - (1.5 * v_std)
        if v_px > v_u: vx_p = 100
        elif v_px < v_l: vx_p = 0
        else: vx_p = 60

    # [5] DeMark: 9-Count (100% if Downside exhausted/Buy)
    dm = (spy > spy.shift(4)).astype(int); lv = int(dm.iloc[-1]); cnt = 0
...
# --- 4. DISPLAY ---
def main():
    st.write("### ALPHA TERMINAL v4.2 // RISK & ALLOCATION LEDGER")
    d = run_model()
    if d is None:
        st.error("SYNC ERROR: Yahoo Finance Throttled. Refresh in 30s.")
        return
    ...
    with c1:
        ...
        st.caption("100% = Maximum Buy Strength | 0% = Maximum Sell/Caution")
    with c2:
    ...
    # THE LEDGER
    st.write("---")
    st.write("**STRATEGIC STRENGTH LEDGER**")
    
    rows = [
        ("Macro: Yield Curve", f"Spread: {d['yc_v']:.2f}% (Watch: {d['yc_w']})", d['yc_p']),
        ("Macro: Global Trend", f"SPY Price vs 200-MA", d['tr_p']),
        ("Macro: Credit Canary", f"HYG/IEF Relative Strength", d['cr_p']),
        ("Int: Sector Breadth", f"{d['br_c']}/11 Sectors > 200-MA", d['br_p']),
    ...
    for label, reading, pct in rows:
        ...
        cb.markdown(f'<div style="display:flex; align-items:center;"><div style="background-color:#30363d; width:100%; height:10px; border-radius:5px; margin-right:15px;"><div style="background-color:{clr}; width:{pct}%; height:10px; border-radius:5px;"></div></div><span class="data-font">{pct}%</span></div>', unsafe_allow_html=True)
